(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('portalApp.httpError', function (event, httpResponse) {
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-portalApp-error');
                var entityKey = httpResponse.headers('X-portalApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('portalApp.' + fieldError.objectName + '.' + convertedField);
